...
namespace KebbiBrain.Hardware
{
    ...
    public static class VoiceCommand
    {
        ...
        {
            if (voice == null || string.IsNullOrEmpty(msg)) return false;
            var p = msg.Split(new[] { D }, 4, StringSplitOptions.None); // 限 4 欄:text(末欄)保留內含的 '|'
            if (p.Length < 2 || p[0] != Prefix) return false;
            // 含空 text 的 "VC|SAY|zh-TW|" 切出 len=4、p[3]="";缺欄位的 "VC|SAY"/"VC|SAY|zh-TW" 切出 len=2/3 → 落到 false(不丟例外)。
            if (p[1] == "SAY" && p.Length >= 4) { _ = voice.SpeakAsync(p[3], Norm(p[2])); return true; }
            return false;
        }
